...
<div class="max-w-md mx-auto mt-16">
    <div class="bg-white p-10 rounded-3xl shadow-soft border border-stone-100">
        <h1 class="text-3xl font-bold text-center mb-8 text-dark">Вход</h1>
        ...
        <form action="{{ route('login') }}" method="POST" class="space-y-5">
            @csrf
            <div>
                <label class="block text-sm font-medium text-stone-700 mb-1">Юзернейм</label>
                <input type="text" name="username" class="w-full p-3 rounded-xl bg-stone-50 border-none focus:ring-2 focus:ring-primary transition" placeholder="username" required value="{{ old('username') }}">
            </div>
        ...
        <p class="text-center mt-6 text-stone-500 text-sm">
            Впервые у нас? <a href="{{ route('register') }}" class="text-primary font-bold hover:underline">Регистрация</a>
        </p>
    </div>
</div>
@endsection